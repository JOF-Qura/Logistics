...
var edit_new_supply = [];
var manual_supply_id_arr = [];
var edit_supply_from_catalog = [];
// new supply array
// non catalog
var new_supply = [];
// catalog
var new_supply_from_catalog = [];

var get_pr_detail_row;
sessionStorage.setItem("supply_temp_id", 0);
  ...
  $("#form_id")
    ...
    .validate({
      ...
      submitHandler: function () {
        ...
        // push all index of new catalog supply to formData["pr_detail"]
        for (let i in new_supply_from_catalog) {
          formData["pr_detail"].push(new_supply_from_catalog[i]);
        }

        // push all index of new non catalog supply to formData["pr_detail"]
        for (var new_supply_index in new_supply) {
          formData["pr_detail"].push(new_supply[new_supply_index]);
        }
        ...
        if (
          ...
          console.log(formDataDetail["pr_details_id"])
          console.log(new_supply_from_catalog)
          console.log(supply_id_array)
          if ($("#uuid").val() == "") {
          //  INSERT 
            $.ajax({
              ...
              success: function (data) {
                ...
                if (data) {
                  ...
                  formReset("hide");
                  supply_id_array.splice(0, supply_id_array.length);
                  new_supply.splice(0, new_supply.length);
                  new_supply_from_catalog.splice(
                    0,
                    new_supply_from_catalog.length
                  );
            ...
            $.ajax({
              ...
              cache: false,
              success: function (data) {
                for (let i in edit_new_supply) {
                  // console.log(edit_new_supply[i]);
                  if (
                    removed_prDetail_arr.indexOf(edit_new_supply[i].id) > -1
                  ) {
                    continue;
                  } else {
                    // update non catalog supplys
                    $.ajax({
                      url:
                        apiURL +
                        "purchase-requisition-detail/" +
                        edit_new_supply[i].id,
                      type: "PUT",
                      // data: form_data,
                      contentType: "application/json",
                      //form_data,
                      data: JSON.stringify({
                        quantity: edit_new_supply[i].quantity,
                        new_category: edit_new_supply[i].new_category,
                        new_supply_name: edit_new_supply[i].new_supply_name,
                        estimated_price: edit_new_supply[i].estimated_price,
                        description: edit_new_supply[i].description,
                      }),
                      dataType: "json",
                      // contentType: false,
                      processData: false,
                ...
                for (let k in edit_supply_from_catalog) {
                  // console.log(edit_supply_from_catalog[k]);
                  if (
                    removed_prDetail_arr.indexOf(
                      edit_supply_from_catalog[k].id
                    ) > -1
                  ) {
                    continue;
                  } else {
                    ...
                    // console.log($(supply_quantity[k]).val());

                    // update catalog supplys
                    $.ajax({
                      url:
                        apiURL +
                        "purchase-requisition-detail/" +
                      ...
                      contentType: "application/json",
                      data: JSON.stringify({
                        quantity: edit_supply_from_catalog[k].quantity,
                      }),
                      dataType: "json",
                      processData: false,
                      cache: false,
                ...
                if (data) {
                  ...
                  supply_id_array.splice(0, supply_id_array.length);
                  new_supply.splice(0, new_supply.length);
                  edit_new_supply.splice(0, edit_new_supply.length);
                  new_supply_from_catalog.splice(
                    0,
                    new_supply_from_catalog.length
                  );

                  edit_supply_from_catalog.splice(
                    0,
                    edit_supply_from_catalog.length
                  );

                  manual_supply_id_arr.splice(0, manual_supply_id_arr.length);
...
productTable = () => {
  ...
  $("#product-table").DataTable({
    ajax: { url: "http://localhost:8000/" + "supplies/active-supplies", dataSrc: "" },
    responsive: true,
    serverSide: false,
    dataType: "json",
    ...
    columns: [
      {
        ...
        render: function (aData, type, row) {
          // let checkbox = "";
          // info
          if (supply_id_array.includes(aData["id"])) {
            return (
              '<input type="checkbox" class="supply_id" value = "' +
              aData["id"] +
              '" checked disabled>'
            );
          } else {
...
addProduct = () => {
  ...
  product_table.$(".supply_id:checkbox:checked:enabled").each(function () {
    console.log(this)
    $.ajax({
      url: "http://localhost:8000/supplies/" + $(this).val(),
      type: "GET",
      dataType: "json",
      headers: {
        Accept: "application/json",
      ...
      success: function (responseData) {
        ...
        if (responseData) {
          ...
          prd_table.row
            ...
            .draw();
          supply_id_array.push(responseData.id);
          supply_id_array.filter(onlyUnique);
          ...
          new_supply_from_catalog.push({
            supply_id: responseData.id,
...
editCatalogProduct = () => {
  ...
  edit_supply_from_catalog.push({
    id: $("#catalog_item_uuid").val(),

    quantity: $("#catalog_item_quantity").val(),
  });

  console.log(edit_supply_from_catalog);
...
addManualProduct = () => {
  ...
  if ($("#new_item_uuid").val() == "") {
    sessionStorage.setItem(
      "supply_temp_id",
      parseInt(sessionStorage.getItem("supply_temp_id")) + 1
    );

    new_supply.push({
      temp_id: sessionStorage.getItem("supply_temp_id"),
    ...
    prd_table.row
      .add([
        ...
        '<div class="text center dropdown"> <div class="btn btn-sm btn-default" data-toggle="dropdown" role="button">' +
          '<i class="fas fa-ellipsis-v"></i></div> <div class="dropdown-menu dropdown-menu-right">' +
          '<div class="dropdown-item d-flex" role="button" data-toggle="modal" onClick="return removeProduct(this.parentNode.parentNode.parentNode.parentNode,this.parentNode.parentNode.parentNode, \'' +
          sessionStorage.getItem("supply_temp_id") +
          '\');"><div style="width: 2rem"> <i class="fas fa-trash mr-1"></i></div><div> Remove</div></div>' +
          "</div></div>",
      ])
      .draw();
  } else {
    edit_new_supply.push({
...
removeProduct = (tr, td, supply_id) => {
  ...
  for (i = 0; i < new_supply.length; i++) {
    if (new_supply[i].temp_id == supply_id) {
      // console.log(new_supply[i].temp_id)
      new_supply.splice(i, 1);
    }    
  }
  for (i = 0; i < new_supply_from_catalog.length; i++) {
    if (new_supply_from_catalog[i].supply_id == supply_id) {
      // console.log(new_supply[i].temp_id)
      new_supply_from_catalog.splice(i, 1);
    }    
  }
  removeA(supply_id_array, supply_id);
};


// remove purchase request items from table
removePrDetail = (tr,td, supply_id, prdetail_id) => {
...
editData = (id, type) => {
  ...
  new_supply_from_catalog.splice(0, new_supply_from_catalog.length);
  edit_new_supply.splice(0, edit_new_supply.length);
  edit_supply_from_catalog.splice(0, edit_supply_from_catalog.length);
    ...
    success: function (data) {
      if (data) {
        ...
        for (var i in data.purchase_requisition_detail) {
          if (data.purchase_requisition_detail[i].supply == null) {
            prd_table.row
              .add([
                // product catagory
            ...
            manual_supply_id_arr.push(data.purchase_requisition_detail[i].id);
            // edit_new_supply.filter(onlyUnique);
            ...
            prd_table.row
              .add([
                // product catagory
                data.purchase_requisition_detail[i].supply.category
                  .category_name,

                // product name
                data.purchase_requisition_detail[i].supply.product_name,

                // product description
                data.purchase_requisition_detail[i].supply.description,

                // estimated price
                "\u20B1" +numberWithCommas(data.purchase_requisition_detail[i].supply.estimated_price),
                ...
                "\u20B1" +numberWithCommas(data.purchase_requisition_detail[i].supply.estimated_price* data.purchase_requisition_detail[i].quantity),
                ...
                '<div class="text center dropdown"> <div class="btn btn-sm btn-default" data-toggle="dropdown" role="button">' +
                  ...
                  // remove
                  '<div class="dropdown-item d-flex" role="button" data-toggle="modal" onClick="return removePrDetail(this.parentNode.parentNode.parentNode.parentNode,this.parentNode.parentNode.parentNode, \'' +
                  data.purchase_requisition_detail[i].supply.id +
                  "', '" +
                  data.purchase_requisition_detail[i].id +
                  '\');"><div style="width: 2rem"> <i class="fas fa-trash mr-1"></i></div><div> Remove</div></div>' +
                  "</div></div>",
            ...
            edit_supply_from_catalog.push({
              id: data.purchase_requisition_detail[i].id,
              supply_id: data.purchase_requisition_detail[i].supply.id,
              product_name:
                data.purchase_requisition_detail[i].supply.product_name,
              quantity: data.purchase_requisition_detail[i].quantity,
            });
            ...
            estimated_total_obj.push({
              id: i,
              price:
                data.purchase_requisition_detail[i].supply.estimated_price,
              quantity: data.purchase_requisition_detail[i].quantity,
              total:
                data.purchase_requisition_detail[i].supply.estimated_price *
                data.purchase_requisition_detail[i].quantity,
      ...
      if (data) {
        ...
        if (data["supply_id"] == null) {
          ...
          $("#catalog_item_category").val(
            data.supply.category["category_name"]
          );
          $("#catalog_item_name").val(data.supply["product_name"]);
          $("#catalog_product_price").val(data.supply["estimated_price"]);

          $("#catalog_item_description").val(data.supply["description"]);
