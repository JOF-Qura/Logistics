...
def get_status(vendor_id, status,db : Session = Depends(get_db)):
...
def get_filtered_po_reports( start_date,end_date,po_status,db : Session = Depends(get_db)):
...
def create(request: PurchaseOrder,db : Session = Depends(get_db)):#
...
def update(id, request: UpdatePurchaseOrderStatus, db : Session = Depends(get_db)):
...
def get_vendor_po(vendor_id, db : Session = Depends(get_db)):
...
def get_orders(id, db : Session = Depends(get_db)):
...
def get_orders_count(vendor_id,db : Session = Depends(get_db)):
...
def vendor_monthly_sales(vendor_id, db : Session = Depends(get_db)):
...
def get_orders_total(vendor_id,db : Session = Depends(get_db)):
