...
def get_replacement_vendor(vendor_id, db : Session = Depends(get_db)):#
    return replacement_request.get_replacement_vendor(vendor_id,db)


# create
@router.post('/', status_code=status.HTTP_201_CREATED)
def create(request: ReplacementRequest, db : Session = Depends(get_db)):#
...
def update(id, request: ReplacementRequestStatus, db : Session = Depends(get_db)):#
