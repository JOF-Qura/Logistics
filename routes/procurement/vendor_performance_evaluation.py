...
def get_ratings( category_id,db : Session = Depends(get_db)):
    return vendor_performance_evaluation.get_ratings(category_id,db)
    
# get all results of specific vendor
@router.get('/charts/vendor-scores/{vendor_id}')
def get_vendor_scores( vendor_id,db : Session = Depends(get_db)):
...
def create(request: VendorEvaluationResult, db : Session = Depends(get_db)):#
    return vendor_performance_evaluation.create(request, db)
    

# delete
@router.delete('/del/{id}', status_code=status.HTTP_202_ACCEPTED)#HTTP_204_NO_CONTENT
def delete(id,db : Session = Depends(get_db)):
    return vendor_performance_evaluation.delete(id,db)
    

# update
@router.put('/{id}',status_code=status.HTTP_202_ACCEPTED)
def update(id, request: VendorEvaluationResult, db : Session = Depends(get_db)):
...
# def get_sched(id, db : Session = Depends(get_db)):
