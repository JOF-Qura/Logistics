...
from repository.procurement import vendor_proposal
from database import get_db

from schemas.procurement.vendor_proposal import AwardVendor, ShowVendorProposal, ShowVendorProposalPayload,VendorProposal,VendorProposalStatus,BiddingItemID,ShowVendorBiddingItems
...
def get_one_bid(proposal_id, db : Session = Depends(get_db)):
...
def get_own_proposal(id, db : Session = Depends(get_db)):
...
def create(request: VendorProposal, db : Session = Depends(get_db)):#
    return vendor_proposal.create(request, db)
...
def update(id, request: VendorProposal, request1: BiddingItemID, db : Session = Depends(get_db)):

    return vendor_proposal.update(id,request, request1, db)


# update status of vendor proposal
@router.put('/status/{id}',status_code=status.HTTP_202_ACCEPTED)
def update_status(id, request: VendorProposalStatus, db : Session = Depends(get_db)):
    return vendor_proposal.update_status(id,request, db)

# award vendor proposal - change status
@router.put('/award/{id}',status_code=status.HTTP_202_ACCEPTED)
def award_vendor(id, request: AwardVendor, db : Session = Depends(get_db)):
    return vendor_proposal.award_vendor(id,request, db)
...
def get_one_bid_item(id, db : Session = Depends(get_db)):
...
def get_awarded(db : Session = Depends(get_db)):
...
def get_proposal_count(user_id,db : Session = Depends(get_db)):
...
def get_filtered_pr_reports( rfq_id,db : Session = Depends(get_db)):
