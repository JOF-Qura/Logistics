...
# def create(product_name: str = Form(...),category_id: str = Form(...), product_pic:UploadFile = File(...), description:str = Form(...),estimated_price:str = Form(...), db : Session = Depends(get_db)):
#     return product.create(product_name,category_id,product_pic,description,estimated_price,db) 
@router.post('/', status_code=status.HTTP_201_CREATED)
def create(product_name: str = Form(...),category_id: str = Form(...), description:str = Form(...),estimated_price:str = Form(...), db : Session = Depends(get_db)):
    return product.create(product_name,category_id,description,estimated_price,db)

# delete
@router.delete('/{id}', status_code=status.HTTP_204_NO_CONTENT)
def delete(id,request: ProductStatus,db : Session = Depends(get_db)):
