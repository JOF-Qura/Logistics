...
from repository.procurement import purchase_requisition
from database import get_db

from schemas.procurement.purchase_requisition import ShowPurchaseRequisition,PurchaseRequisition,PurchaseRequisitionStatus, PrDetailID
# from schemas.procurement.user import User
from schemas.procurement.pr_timeline import ShowPurchaseRequisitionTimeline
...
def get_pending(department_id, db : Session = Depends(get_db)):
...
def get_pending_cancelled(department_id, db : Session = Depends(get_db)):
...
def get_approved(department_id, db : Session = Depends(get_db)):
...
def get_filtered_pr_reports( start_date,end_date,pr_status,db : Session = Depends(get_db)):
    return purchase_requisition.get_filtered_pr_reports(start_date,end_date,pr_status,db)


# create purchase requisition
@router.post('/', status_code=status.HTTP_201_CREATED)
def create(request: PurchaseRequisition, db : Session = Depends(get_db)):#
    return purchase_requisition.create(request, db)
...
def update(id, request: PurchaseRequisition, db : Session = Depends(get_db)):
    
    return purchase_requisition.update(id,request, db)


# update status of purchase requisition
@router.put('/status/{id}',status_code=status.HTTP_202_ACCEPTED)
def update_status(id, request: PurchaseRequisitionStatus, db : Session = Depends(get_db)):
...
def get_pr_timeline(id, db : Session = Depends(get_db)):
...
def get_draft_status(department_id, db : Session = Depends(get_db)):
...
def get_status(department_id, db : Session = Depends(get_db)):
...
def get_count( department_id,db : Session = Depends(get_db)):
...
def get_pr_categ(department_id, db : Session = Depends(get_db)):
...
def get_pr_per_month( db : Session = Depends(get_db)):
