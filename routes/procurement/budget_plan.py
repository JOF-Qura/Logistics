...
def get_budget(year, db : Session = Depends(get_db)):
...
def get_one_budget(department_id, year, db : Session = Depends(get_db)):
...
def get_dept_spent(department_id,year, db : Session = Depends(get_db)):
...
def get_budget_department(id, db : Session = Depends(get_db)):
...
def update_budget(id, request: BudgetPlan, db : Session = Depends(get_db)):
    return budget_plan.update(id, request, db)


# update status - activate / deactivate
@router.put('/{id}',status_code=status.HTTP_202_ACCEPTED)
def update_budget_status(id: str,db : Session = Depends(get_db) ):
...
def update_budget_po(given_budget:float,total_amount:float,year,department_id, db : Session = Depends(get_db), ):
