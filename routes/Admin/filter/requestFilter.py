...
# GET All request by Requestor
@router.get('/datatable/filter_to_procurement_pending')
def datatableProcurement(request: Request, db: Session = Depends(get_db)):
    try:
        def perform_search(queryset, user_input):
            return queryset.filter(
        ...
        table = DataTable(dict(request.query_params), Request_M, db.query(Request_M).filter(requestModel.Request.requestor == "Procurement", requestModel.Request.request_status == "On Process"), 
        [
            'request_id',
            'request_date',
            'requestor',
...
# GET All request by Type
@router.get('/datatable/filter_to_for_request_type_pending')
def datatableWarehouse(request: Request, db: Session = Depends(get_db)):
    try:
        def perform_search(queryset, user_input):
            return queryset.filter(
        ...
        table = DataTable(dict(request.query_params), Request_M, db.query(Request_M).filter(requestModel.Request.request_type == "For Request", requestModel.Request.request_status != "Pending"), 
        [
            'request_id',
            'request_date',
            'requestor',
